...
# async def editar_agendamento(request: Request, agendamento_id: int, nome: str = Form(...), data: str = Form(...), hora: str = Form(...), db: Session = Depends(get_db)):
#     user_email = request.session.get('user_email')
#     if not user_email:
#         return RedirectResponse(url="/login", status_code=303)
...
# @app.post("/editar_agendamento/{agendamento_id}", response_class=HTMLResponse)
# async def editar_agendamento(request: Request, agendamento_id: int, db: Session = Depends(get_db)):
#     user_email = request.session.get('user_email')
#     if not user_email:
#         return RedirectResponse(url="/login", status_code=303)
...
#     # Redirecionar de volta para a página de meus agendamentos
#     return RedirectResponse(url="/meus_agendamentos", status_code=303)
